...
export default class AdminController extends Controller {
  ...
  public async forgotPassword(
    ...
    try {
      ...
      let url = process.env.REACT_APP_BASE_PATH;
      // if(mode === 'DEV') {
      ...
      await sendEmail(
        ...
        "Reset Your Password",
        email,
        template({ link: `${url}reset-password?resetId=${token}` })
      );
      return {
        data: {},
        error: "",
  ...
  public async resetPassword(
    @Body() request: { password: string; confirmPassword: string }
  ): Promise<IResponse> {
    try {
      console.log(this.userId);

      const { password, confirmPassword } = request;
      const validatedResetPassword = validateResetPassword({
        password,
        confirmPassword,
      });
